...
public class PagesViews {
    //Get page views for the last 7 days, per platform.
    // Main class should be named 'Solution'
    static class Solution {
        public static void main(String[] args) {
            System.out.println("We are the best team ever");
            for (Map.Entry<String, DateEntry> entry:
                    filterMethod().entrySet()
                 ) {
                System.out.println(entry.getKey());
                System.out.println("Andriod page views:" + entry.getValue().android.pageViews);
                System.out.println("iOS page views:" +entry.getValue().ios.pageViews);
                System.out.println("Mobile web page views:" +entry.getValue().mobile_web.pageViews);
                System.out.println("New reddit page views:" +entry.getValue().new_reddit.pageViews);
                System.out.println("Old reddit page views:" +entry.getValue().old_reddit.pageViews);
        ...
        static Map<String, DateEntry> filterMethod(){
            LocalDate today=LocalDate.now();
            LocalDate exampleDate = LocalDate.parse("2020-01-21").minus(7, ChronoUnit.DAYS);


            Map<String,DateEntry> inputMap=generateDataInMapFormat();
            Map<String,DateEntry> lastWeekMap= new HashMap<>();

            lastWeekMap=inputMap.entrySet().stream().filter(e->!e.getKey().isBlank())
                    .filter(e->LocalDate.parse(e.getKey()).isAfter(exampleDate))
                    .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

            //lastWeekMap=inputMap.entrySet().stream().filter(e->!e.getKey().isBlank()).filter(e->LocalDate.parse(e.getKey()).compareTo(today)>=-7).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        return lastWeekMap;
        }
